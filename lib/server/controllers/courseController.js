...
exports.submitAnswer = async (req, res) => {
    try {
        ...
        // Проверяем, сдавался ли уже этот урок
        if (user.answers.some(ans => ans.lessonId === lessonId)) {
            return res.status(409).json({ message: 'Lesson already completed' });
        }

        // Сохраняем ответы
        user.answers.push({ lessonId, submittedAt: new Date(), score, teacherFeedback: '', answers, projectLinks });
        ...
        // === Работаем с Map ===
        if (!user.progress) {
            // Инициализируем пустой Map, если его нет
            user.progress = new Map();
        }
        if (!user.progress.has(courseKey)) {
            // Создаём структуру для этого курса
        ...
        // Добавляем номер урока
        if (!progress.lessonsCompleted.includes(lessonId)) {
            progress.lessonsCompleted.push(lessonId);
        }

        // Отметка по викторине
        ...
        // Всегда помечаем поле progress как изменённое
