...
const AdminNewsSchema = new mongoose.Schema({
    ...
    image: { type: String }, // URL картинки (опционально)
...
app.post('/admin/news', async (req, res) => {
    ...
    try {
        ...
        const news = new AdminNewsModel({ title, description, image });
        await news.save();
...
app.get('/news', async (req, res) => {
    try {
        const adminNews = await AdminNewsModel.find().sort({ createdAt: -1 });
        res.json(adminNews);
    } catch (error) {
...
app.put('/admin/update-user', async (req, res) => {
    const { email, username, birthDate, city, phone, twoFactorEnabled } = req.body;
    const user = await UserModel.findOne({ email });
    if (!user) return res.status(404).json({ message: 'User not found' });
...
app.post('/teacher/approve-answer', async (req, res) => {
    ...
    try {
        ...
        const answer = user.answers.find(ans => ans.lessonId === lessonId);
